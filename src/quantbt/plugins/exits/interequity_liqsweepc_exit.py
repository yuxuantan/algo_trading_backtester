...
from quantbt.plugins.entries.interequity_liqsweepc_entry import get_cached_bracket
from quantbt.plugins.registry import register_exit


@register_exit("interequity_liqsweepc_exit")
def build_exit(side: str, entry_open: float, prev_low: float, prev_high: float, params: dict, entry=None):
    """
    InterEquity LiqSweepC exit plugin.
