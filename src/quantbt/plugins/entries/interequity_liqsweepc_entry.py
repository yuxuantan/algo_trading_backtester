...
_STRAT_MODULE = "quantbt.strategies.interequity_2026_03_liqsweep_c"
...
@register_entry("interequity_liqsweepc_entry")
def signals(df: pd.DataFrame, params: dict) -> pd.DataFrame:
    """
    Entry plugin backed by the full InterEquity LiqSweepC state-machine.

    LiqSweepC enters on the sweep bar itself. The generic plugin engine can only
    enter on the next bar open, so we emit the signal on the strategy entry bar
    and map cached brackets to the next timestamp.
    """
