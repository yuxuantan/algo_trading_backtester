...
from quantbt.strategies.interequity_2026_03_liqsweep_a import (
    ...
    _LevelPool,
    _build_entry_spec,
    _build_entry_spec_from_exit_override,
...
def _arm_reclaim(
    current: _PendingReclaim | None,
    *,
    swept_level: float | None,
    current_index: int,
    max_index: int,
) -> _PendingReclaim | None:
    del current
    if swept_level is None:
        return None
    expiry = min(int(max_index), int(current_index) + int(RECLAIM_WINDOW_BARS))
    return _PendingReclaim(
        level=float(swept_level),
        sweep_index=int(current_index),
        expiry_index=int(expiry),
    )


def _resolve_reclaim(
    pending: _PendingReclaim | None,
    *,
    current_index: int,
    close_value: float,
    side: str,
) -> tuple[_PendingReclaim | None, float | None]:
    if pending is None:
        return None, None
    if int(current_index) > int(pending.expiry_index):
        return None, None
    if int(current_index) < int(pending.sweep_index):
        return pending, None

    if side == "short":
        reclaimed = float(close_value) < float(pending.level)
    elif side == "long":
        reclaimed = float(close_value) > float(pending.level)
    else:
    ...
    if reclaimed:
        return None, float(pending.level)
    return pending, None
...
def run_backtest(
    ...
    *,
    ...
    pending_short_reclaim: _PendingReclaim | None = None
    pending_long_reclaim: _PendingReclaim | None = None
    ...
    for i, t in enumerate(idx):
        ...
        if bool(p.show_levels):
            ...
            swept_short_price, swept_long_price = sweep_triggers(
                high_pool,
                low_pool,
                h,
                l,
            ...
            if pos is None:
                new_short_reclaim = _arm_reclaim(
                    pending_short_reclaim,
                    swept_level=swept_short_price,
                    current_index=i,
                    max_index=len(df) - 1,
                )
                if new_short_reclaim is not None:
                    pending_short_reclaim = new_short_reclaim
                new_long_reclaim = _arm_reclaim(
                    pending_long_reclaim,
                    swept_level=swept_long_price,
                    current_index=i,
                    max_index=len(df) - 1,
                )
                if new_long_reclaim is not None:
                    pending_long_reclaim = new_long_reclaim

        pending_short_reclaim, short_entry_price = _resolve_reclaim(
            pending_short_reclaim,
            current_index=i,
            close_value=c,
            side="short",
        )
        pending_long_reclaim, long_entry_price = _resolve_reclaim(
            pending_long_reclaim,
            current_index=i,
            close_value=c,
            side="long",
        ...
        if opened_this_bar:
            pending_short_reclaim = None
            pending_long_reclaim = None
